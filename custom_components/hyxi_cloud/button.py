    ...
    async def async_press(self) -> None:
        ...
        try:
            if self._mode == "idle":
                ...
                _block_manual_self_consume_if_needed(self.coordinator, self._sn)
...
def _block_manual_self_consume_if_needed(coordinator, sn: str) -> None:
    """Reject manual self-consume when SOC protection says it is unsafe."""
    if (
        controller := _get_protection_controller(coordinator, sn)
    ) is not None and controller.should_block_manual_self_consume():
        raise HomeAssistantError(
            "Self-consume blocked because battery SOC is at or below SOC Minimum"
        )
...
def _block_manual_peak_shaving_if_needed(coordinator, sn: str, option: str) -> None:
    ...
    if option == "hold" and controller.should_block_manual_hold():
        raise HomeAssistantError("Peak shaving hold blocked by battery SOC protection")
