...
Manages battery charging and operating mode based on:
  ...
  - Battery SOC limits (configurable min/max)
...
from .const import DOMAIN, EM_DEFAULTS, EM_LOOP_INTERVAL
...
# Rolling average window for P1 readings (1 minute of samples)
_P1_AVG_WINDOW = 60
...
class EnergyManagerEngine:
    ...
    def __init__(
        ...
        self._forecast_power_entity = forecast_power_entity
    ...
    async def async_start(self) -> None:
        ...
        self._enabled = True
        _LOGGER.info("Energy Manager started for %s", self._sn)

        # 15-second decision loop
        self._unsub_listeners.append(
            async_track_time_interval(
                self._hass,
                self._loop_tick,
                timedelta(seconds=EM_LOOP_INTERVAL),
            )
        )
    ...
    async def async_stop(self) -> None:
        ...
        _LOGGER.info("Energy Manager stopped for %s", self._sn)
        self._notify_sensors()
    ...
    async def _make_decision(self) -> None:
        ...
        if solar_producing and soc < soc_max:
            if self._current_mode != "charge":
                # Default: stay in self_consume, only charge on sustained export
                ...
                # In charge mode: fine-tune, cap at solar, exit if sustained import
