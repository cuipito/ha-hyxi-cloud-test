...
def test_manual_self_consume_allowed_during_upper_hold():
    """Three-phase self-consume should remain allowed during the upper hold."""
    controller = _build_controller(89)
    controller._high_soc_hold = True

    assert controller.should_block_manual_self_consume() is False
...
def test_single_phase_manual_self_consume_blocked_at_soc_min():
    """Single-phase self-consume equivalent should still be blocked at low SOC."""
    controller = _build_controller(20, "H5K-HS")

    assert controller.should_block_manual_self_consume() is True
